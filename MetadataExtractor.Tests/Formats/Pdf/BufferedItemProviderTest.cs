...
{
    /// <summary>Unit tests for <see cref="BufferedItemProvider{ItemType}"/>.</summary>
    /// <author>Drew Noakes https://drewnoakes.com</author>
    public sealed class BufferedItemProvider
    {
        private static BufferedItemProvider<byte> GetProvider(int startIndex, int bufferLength)
        {
            string input = @"123456789";

            StringByteProviderSource byteSource = new StringByteProviderSource(input, startIndex, ExtractionDirection.Forward);

            return new BufferedItemProvider<byte>(byteSource, bufferLength);
        }
        ...
        {
            ItemProvider<byte> byteProvider = GetProvider(startIndex: 0, bufferLength: 9);
            ...
            // buffer length less than 9

            byteProvider = GetProvider(startIndex: 0, bufferLength: 2);
        ...
        {
            ItemProvider<byte> byteProvider = GetProvider(startIndex: 2, bufferLength: 7);
            ...
            byteProvider = GetProvider(startIndex: 5, bufferLength: 2);
        ...
        {
            ItemProvider<byte> byteProvider = GetProvider(startIndex: 2, bufferLength: 2);
        ...
        {
            ItemProvider<byte> byteProvider = GetProvider(startIndex: 2, bufferLength: 3); // bufferLength must be > 2, since we peek with delta=2
        ...
        [Fact]
        public void TestCurrentIndexStartIndexNonZero()
        {
            ItemProvider<byte> byteProvider = GetProvider(startIndex: 2, bufferLength: 3); // bufferLength must be > 2, since we peek with delta=2
